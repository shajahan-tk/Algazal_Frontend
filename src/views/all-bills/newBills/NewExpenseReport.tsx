...
import ExpenseReportForm from '../billForms/ExpenseReportForm'
import {  editAdibReportAndExpenses, addAdibReportAndExpenses, fetchAdibReportAndExpensesById } from '../api/api'
...
const NewExpenseReport = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                toast.push(
                    <Notification
                        title="Failed to fetch expense report data"
                        type="danger"
                        duration={2500}
                    >
                ...
                setTimeout(() => navigate('/app/expense-report-view'), 2500)
            } finally {
                setLoading(false)
            }
        }
    ...
    const handleFormSubmit = async (
        ...
        try {
            ...
            formDataToSend.append('reportType', 'expense')
            ...
            if ([200, 201].includes(response.status)) {
                toast.push(
                    <Notification
                        title={`Successfully ${id ? 'updated' : 'added'} expense report`}
                        type="success"
                        duration={2500}
                    >
                        Expense report {id ? 'updated' : 'added'} successfully
                    </Notification>,
                    { placement: 'top-center' }
                )
                navigate('/app/expense-report-view')
            } else {
            ...
            toast.push(
                <Notification
                    title={`Error ${id ? 'updating' : 'adding'} expense report`}
                    type="danger"
                    duration={2500}
                >
    ...
    if (error) {
        return (
            <div className="flex justify-center items-center h-64">
                <Notification
                    title="Error loading expense report"
                    type="danger"
                >
                    {error}
                </Notification>
